...
import {
    realCostTrackingService,
    RealCostItem,
} from '@/lib/business/real-cost-tracking';
...
export function CostOptimizationPanel({onOptimizationApplied}: CostOptimizationPanelProps) {
    ...
    const generateOptimizationOpportunities = (costs: RealCostItem[], summary: any): OptimizationOpportunity[] => {
        const opportunities: OptimizationOpportunity[] = [];
        
        // Analyze each cost category for optimization opportunities
        for (const cost of costs) {
            const variance = cost.projectedMonthlyCost > 0 ? ((cost.actualMonthlyCost - cost.projectedMonthlyCost) / cost.projectedMonthlyCost) * 100 : 0;
                        ...
                        <OptimizationCard key={optimization.id} $priority={optimization.priority}>
                            <Stack spacing="md">
                                ...
                                <Stack spacing="xs">
                                    ...
                                    <Badge
                                            variant={optimization.priority === 'high' ? 'error' : optimization.priority === 'medium' ? 'warning' : 'success'}
                                            size="sm"
                                        >
