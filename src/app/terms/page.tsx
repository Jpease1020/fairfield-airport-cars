...
export default function TermsPage() {
  ...
  const termsContent = cmsConfig?.pages?.terms;
  ...
  const handleCancel = () => {
    setLocalContent(JSON.parse(JSON.stringify(termsContent)));
    setEditMode(false);
    setSaveMsg(null);
  };
    ...
    <PageContainer maxWidth="2xl" padding="lg">
      ...
      <PageContent>
        {editMode ? (
          ...
          <div>
            <h2 className="text-2xl font-bold mb-4">{termsContent?.title || 'Terms of Service'}</h2>
            <div className="prose max-w-none text-gray-700 whitespace-pre-line">{termsContent?.content || 'Terms and conditions go here.'}</div>
          </div>
        )}
      </PageContent>
    </PageContainer>
