...
class FirebaseTrackingService {
    ...
    private async loadGoogleMapsAPI(): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            script.onerror = () => reject(Error('Failed to load Google Maps API'));
    ...
    async calculateAdvancedETA(bookingId: string, pickupLocation: string, dropoffLocation: string, currentLocation?: DriverLocation): Promise<ETACalculation> {
        try {
            ...
            const routeData = await this.calculateRouteWithTraffic(currentLocation || {
                lat: 0,
                lng: 0,
                timestamp: new Date(),
                heading: 0,
                speed: 0,
            }, pickupLocation, dropoffLocation, trafficConditions);
            ...
            console.error('Failed to calculate advanced ETA', error);
            throw error;
        }
    }
    ...
    private getTimeBasedTrafficConditions(): 'low' | 'medium' | 'high' {
        ...
        if ((hour >= 6 && hour <= 10) || (hour >= 15 && hour <= 20))
            return 'medium';
        
        return 'low';
    }
    // Calculate route with traffic awareness using Google Maps API
    private async calculateRouteWithTraffic(currentLocation: DriverLocation, pickup: string, dropoff: string, trafficConditions: 'low' | 'medium' | 'high'): Promise<{
        distance: number;
        duration: number;
        polyline: string;
    }> {
        try {
            ...
            console.error('Failed to calculate route with traffic', error);
            return this.calculateFallbackRoute(pickup, dropoff, trafficConditions);
        }
    }
    // Fallback route calculation
    private calculateFallbackRoute(pickup: string, dropoff: string, trafficConditions: 'low' | 'medium' | 'high'): {
        distance: number;
        duration: number;
        polyline: string;
    } {
        ...
        const baseDuration = baseDistance * 2;
