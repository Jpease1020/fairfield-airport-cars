...
class DriverLocationService {
    ...
    private handleDriverUpdate(driverId: string, data: any): void {
        try {
            ...
            console.error('Failed to handle driver update', error);
        }
    }
    ...
    async getAvailableDriversNearLocation(latitude: number, longitude: number, radiusMiles: number = 10): Promise<DriverStatus[]> {
        try {
            // TODO: Implement geospatial query with Firebase GeoFirestore
            // For now, get all available drivers
            const driversQuery = query(collection(db, 'drivers'), where('status', '==', 'available'), orderBy('lastUpdated', 'desc'), limit(20));
            
            const driversSnapshot = await getDocs(driversQuery);
            const drivers: DriverStatus[] = [];
            
            for (const snap of driversSnapshot.docs) {
                const data = snap.data();
                
                if (data.currentLocation) {
                    // Calculate distance (simplified - should use proper geospatial query)
                    const distance = this.calculateDistance(latitude, longitude, data.currentLocation.lat, data.currentLocation.lng);
            ...
            console.error('Failed to get available drivers near location', error);
            return [];
        }
    }
    ...
    private calculateDistance(lat1: number, lon1: number, lat2: number, lon2: number): number {
        ...
        const a = Math.sin(dLat / 2) ** 2 + Math.cos(this.toRadians(lat1)) * Math.cos(this.toRadians(lat2)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
