...
export const DateTimePicker: React.FC<DateTimePickerProps> = ({
  ...
  // Convert ISO string to Date object using local timezone
  const selectedDate = value ? new Date(value.replace('Z', '')) : null;
  ...
  const handleDateChange = (date: Date | null) => {
    if (date && onChange) {
      // Normalize to local ISO without timezone offset
  ...
  const inputName = useMemo(
    () => (id ? `${id}-display` : `dt-${Math.random().toString(36).slice(2)}`),
    [id]
  );
  ...
  return (
    <DatePickerWrapper 
      ...
      data-testid={cmsId}
    >
      {/* Hidden input to discourage browser autofill */}
      <input
        type="text"
        tabIndex={-1}
